...
// the constructor
Port::Port(QDomNode &portNode): name_(), portType_(General::WIRE),
wire_(),
transactional_(), portAccessHandle_(), portAccessType_() {
	...
	for (int i = 0; i < portNode.childNodes().count(); ++i) {
		...
		if (tempNode.nodeName() == QString("spirit:name") ||
				tempNode.nodeName() == QString("spirit:logicalName")) {
...
// Function: Port()
//-----------------------------------------------------------------------------
Port::Port(QString const& name, Port const& other) : name_(name), portType_(other.portType_),
                                                     wire_(), transactional_(other.transactional_),
...
Port::Port(): name_(), portType_(General::WIRE),
wire_(),
transactional_(), portAccessHandle_(), portAccessType_()  {
